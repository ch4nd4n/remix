    ...
    async setup(build) {
      ...
      build.onLoad({ filter: /\.mdx?$/ }, async args => {
        try {
          ...
          switch (typeof config.buildOptions.mdx) {
            case "object":
              rehypePlugins.push(
                ...(config.buildOptions.mdx.rehypePlugins || [])
              );
              remarkPlugins.push(
                ...(config.buildOptions.mdx.remarkPlugins || [])
              );

              break;
            case "function":
              let mdxConfig = await config.buildOptions.mdx(args.path);
